...
__device__ static void both_const_idx(const uint16_t* src, ptrdiff_t src_stride, uint16_t* dst, ptrdiff_t dst_stride,
	...
	for (y = 0; y < 11; ++y)
	{
		const uint16_t* src_x = &ref_row[0];
	...
	dst[0 * dst_stride + 0] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[0], FILTER_BITS), bd);
	dst[1 * dst_stride + 0] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[4], FILTER_BITS), bd);
	dst[2 * dst_stride + 0] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[8], FILTER_BITS), bd);
	dst[3 * dst_stride + 0] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[12], FILTER_BITS), bd);

	dst[0 * dst_stride + 1] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[1], FILTER_BITS), bd);
	dst[1 * dst_stride + 1] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[5], FILTER_BITS), bd);
	dst[2 * dst_stride + 1] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[9], FILTER_BITS), bd);
	dst[3 * dst_stride + 1] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[13], FILTER_BITS), bd);

	dst[0 * dst_stride + 2] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[2], FILTER_BITS), bd);
	dst[1 * dst_stride + 2] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[6], FILTER_BITS), bd);
	dst[2 * dst_stride + 2] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[10], FILTER_BITS), bd);
	dst[3 * dst_stride + 2] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[14], FILTER_BITS), bd);
	
	dst[0 * dst_stride + 3] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[3], FILTER_BITS), bd);
	dst[1 * dst_stride + 3] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[7], FILTER_BITS), bd);
	dst[2 * dst_stride + 3] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[11], FILTER_BITS), bd);
	dst[3 * dst_stride + 3] = cuda_clip_pixel_highbd(ROUND_POWER_OF_TWO(sm[15], FILTER_BITS), bd);
}
...
__device__ static void cuda_highbd_inter_predictor_comp(const uint16_t* src, int src_stride, uint16_t* dst, int dst_stride,
	const int subpel_x, const int subpel_y, int w, int h, const InterpKernel* kernel, int xs, int ys, int bd, int y0, int frame_h)
{
#if CONFIG_VP9_HIGHBITDEPTH
	cuda_vpx_highbd_convolve8_avg_c(src, src_stride, dst, dst_stride, kernel, subpel_x, xs, subpel_y, ys, w, h, bd, y0, frame_h);
#else
#endif
}
...
__device__ static void cuda_highbd_inter_predictor_both(const uint16_t* src, int src_stride, uint16_t* dst, int dst_stride,
	const int subpel_x, const int subpel_y, const InterpKernel* kernel, int xs, int ys, int bd, int y0, int frame_h)
{
#if CONFIG_VP9_HIGHBITDEPTH
	cuda_highbd_convolve(src, src_stride, dst, dst_stride, kernel, subpel_x, xs, subpel_y, ys, bd, y0, frame_h);
#else
#endif
}
...
__device__ static void cuda_extend_and_predict_comp(const uint8_t* buf_ptr1, int pre_buf_stride, int x0, int y0, int b_w,
	...
	cuda_highbd_inter_predictor_comp(mc_buf_high + border_offset, b_w, CONVERT_TO_SHORTPTR(dst), dst_buf_stride, subpel_x,
		subpel_y, w, h, kernel, xs, ys, bd, 0, 0);
#else
...
__device__ static void cuda_dec_build_inter_predictors_comp(int t_subsampling_x, int t_subsampling_y, int mb_to_top_edge, 
	int mb_to_left_edge, int bit_depth, int x, int y, int w, int h, const InterpKernel* kernel,
	int buf_stride, uint8_t* dst_buf, const int row, const int col, uint8_t* ref_frame, int frame_height) {
	...
	cuda_highbd_inter_predictor_comp(CONVERT_TO_SHORTPTR(buf_ptr), buf_stride,
		...
		subpel_y, w, h, kernel, xs, ys, bit_depth, y0, frame_height);
#else
...
__device__ static void cuda_dec_build_inter_predictors_4x4_comp(FrameInformation* fi, int subsampling_x,
	...
	cuda_dec_build_inter_predictors_comp(subsampling_x, subsampling_y, -(mi_row * MI_SIZE * 8),
		...
		buf_stride, dst_buf, my, mx, ref_frame_buf, frame_height);
}
                          ...
                          VP9_COMMON* cm, VP9Decoder* pbi, int tile_rows, int tile_cols)
{
	...
	*gpu_copy = (double)(copy_end - copy_begin) / CLOCKS_PER_SEC;
	...
	//
	int a = 0; // copy + vert + horiz;
	int blocksPerGrid4 = MAX(1, (super_size + threadsPerBlock - 1) / threadsPerBlock);
	cuda_inter_4x4_both <<<blocksPerGrid4, threadsPerBlock>>> (cd_alloc, frame_ref1, frame_ref2, frame_ref3, cd_fi, mv,
		super_size, ref_frame, block_settings, a);
